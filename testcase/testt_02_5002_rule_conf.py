...
class TestRuleCof(object):
    ...
    策略应用
    """
    testdata_file = os.path.join(DATA_DIR, "../testdata/test_02_5002_rule_cof.yaml")
    test_data = YamlUtil(testdata_file).read_yaml()["case_10000"]

    @pytest.mark.smoke_jixian
    @pytest.mark.parametrize("data", test_data)
    @allure.title("策略配置_配置自定义策略，命中策略策略告警-高风险的测试")
    # @allure.story("")
    def test_01(self, data):
        ...
        if RuleAddInterface().request_findrule(data["rulesName"], data["ruleName"]) == 0:
            ...
            with allure.step("#c.检查是否添加成功"):
                ...
                assert res != 0
        # 设置策略动作
        with allure.step("#1.设置规则为通过-高风险规则"):
            res = RuleAddInterface().request_rule_edit_action(data["rulesName"], data["ruleName"], "1")
            assert res["data"] == "success"
