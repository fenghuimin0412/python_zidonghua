...
class TestRuleCof(object):
    ...
    策略应用
    """
    testdata_file = os.path.join(DATA_DIR, "../testdata/test_02_5005_rule_cof.yaml")
    test_data = YamlUtil(testdata_file).read_yaml()["case_10000"]

    @pytest.mark.smoke_jixian
    @pytest.mark.parametrize("data", test_data)
    @allure.title("策略配置_资源应用虚拟补丁，执行于九月命中策略测试")
    # @allure.story("")
    def test_01(self, data):
        ...
        """
        # 设置策略动作：虚拟补丁
        with allure.step("#1.为数据资源应用虚拟补丁策略"):
            # 策略虚拟补丁策略
            res = RuleConfFace().request_patch_conf(data["sourceName"])
        ...
        with allure.step("#3.获取最新10秒内日志，检索到一条操作为：select id from test_rjy.test的审计日志"):
            ...
            while a < 12:
                ...
                a = a+1
            # ruleLevels =3  高风险
            assert res == 3 and ressql == data["strsql"]
